...
with mod_base:
    idata_base = pm.sample(1000, random_seed=33)
    ppc_base = pm.sample_posterior_predictive(idata_base)

base_summ = az.summary(ppc_base, hdi_prob=0.9)    
base_summ_like = az.summary(ppc_base.posterior_predictive, hdi_prob=0.9)   
base_summ = pd.concat([base_summ, base_summ_like]) 
base_summ.to_csv("mod1_posterior_predictive_summary.csv")

ppc_base = ppc_base.stack(sample = ['chain', 'draw']).posterior_predictive



###### Plot posterior predictive for base model (model 1)

h_ppc_high = ppc_base['yh'][0,:,:].values
h_ppc_low = ppc_base['yh'][1,:,:].values

f_ppc_high = ppc_base['yf'][0,:,:].values
f_ppc_low = ppc_base['yf'][1,:,:].values
...
axs[0,0].text(s="Model 1 (Base Model)", x=-10, y=60, size=20)
plt.tight_layout()
plt.savefig("mod1_posterior_predictives.png", dpi=300)
plt.show()
...
with mod_var:
    idata_var = pm.sample(1000, random_seed=33)
...
var_summ.to_csv("mod2_posterior_predictive_summary.csv")   
...
axs[0,0].text(s="Model 2 (Varying Model)", x=-10, y=60, size=20)
plt.tight_layout()
plt.savefig("mod2_posterior_predictives.png", dpi=300)
plt.show()
...
with mod_lkj:
    idata_lkj = pm.sample(1000, random_seed=33)
...
lkj_summ.to_csv("mod3_posterior_predictive_summary.csv")  
...
#axs[1,1].set_title("Group 2 (low)")
axs[1,1].spines[['right', 'top']].set_visible(False)
axs[0,0].text(s="C", x=-20, y=60, size=24)
axs[0,0].text(s="Model 3 (LKJ Model)", x=-10, y=60, size=20)
plt.tight_layout()
plt.savefig("mod3_posterior_predictives.png", dpi=300)
plt.show()
