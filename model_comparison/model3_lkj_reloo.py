...
def compile_mod(hit, signal, fa, noise):
    ...
    with pm.Model(coords=coords) as model:
        ...
        high = pm.MvNormal("high", mu=high_means, cov=high_cov, shape=(p,g))#, shape=(p,g))
        ...
        low = pm.MvNormal("low", mu=low_means, cov=low_cov, shape=(p,g))#, shape=(p,g))
        ...
        hid = hit_id.shape[0].eval()
        fid = fa_id.shape[0].eval()
        
        H = pm.Deterministic('H', Phi(0.5*d - c)).flatten()[:hid] # hit rate 
        F = pm.Deterministic('F', Phi(-0.5*d - c)).flatten()[:fid] # false alarm rate
...
sample_kwargs = {"draws":1000, "tune":1000, "chains":4, "cores":12, "random_seed":33}#, "cores":1, "init":'advi'}
...
reloo_df.to_csv("mod3_reloo.csv")

reloo_k = loo_relooed.pareto_k
k_df = pd.DataFrame(reloo_k)
k_df.to_csv("mod3_k_hats.csv")
az.plot_khat(loo_relooed)
plt.savefig('mod3_khat.png', dpi=300)
plt.close()

loo_i = loo_relooed.loo_i
np.save("mod3_loo_i.npy", loo_i)

file = open("mod3_reloo.obj","wb")
pickle.dump(loo_relooed,file)
file.close()

log = open("mod3_reloo_summary.txt", 'a')
def oprint(message):
    print(message)
    global log
    log.write(message)
...
log = open("mod3_loo_orig_summary.txt", 'a')
def oprint(message):
    print(message)
    global log
    log.write(message)
