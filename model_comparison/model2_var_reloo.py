...
reloo_df.to_csv("mod2_reloo.csv")

reloo_k = loo_relooed.pareto_k
k_df = pd.DataFrame(reloo_k)
k_df.to_csv("mod2_k_hats.csv")
az.plot_khat(loo_relooed)
plt.savefig('mod2_khat.png', dpi=300)
plt.close()

loo_i = loo_relooed.loo_i
np.save("mod2_loo_i.npy", loo_i)

file = open("mod2_reloo.obj","wb")
pickle.dump(loo_relooed,file)
file.close()

log = open("mod2_reloo_summary.txt", 'a')
def oprint(message):
    print(message)
    global log
    log.write(message)
...
log = open("mod2_loo_orig_summary.txt", 'a')
def oprint(message):
    print(message)
    global log
    log.write(message)
